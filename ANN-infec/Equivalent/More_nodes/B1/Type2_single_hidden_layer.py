...
#to setup the initial h_elem
    for k in range (1,2):
        ...
        for j in range ((hidden_layer+1),input_layer):
            elem=tf.constant([[1.]])
            O=tf.concat([O,elem],1)
        D=tf.diag(O[0])
        h_elem=tf.matmul(X,D)
        h_elem=tf.matmul(h_elem,W[:,(k-1):k])
        h=h_elem
	#print(h_elem.shape)


# use the loop to seup the matrix h
    for k in range (2,(hidden_layer+1)):
        O=tf.constant([[1.]]) # first element for bias
        for i in range (1,(hidden_layer+1)):
            if i==k:
                elem=tf.constant([[1.]])
...
def main():
    ...
    updates = tf.train.GradientDescentOptimizer(0.01).minimize(cost) # oringinal 0.01
