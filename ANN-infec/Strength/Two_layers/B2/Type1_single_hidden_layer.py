...
#temporarily set weights matrix for additional edition later
    ...
    diff=input_layer-hidden_layer*5

#setup the diagnal matrix D and do the multiplication
#to setup the initial h_elem
    for k in range (0,1): # k: represent the column
        O=tf.constant([[1.]]) # first element for bias
        for i in range (1,diff):    # i: represent the row
            elem=tf.constant([[1.]])
            O=tf.concat([O,elem],1)
        for j in range (diff,input_layer): # j: represent the row
            if (j-diff)//5==k:
                elem=tf.constant([[1.]])
            else:
                elem=tf.constant([[0.]])
...
def main():
    ...
    # Layer's sizes
    x_size = train_X.shape[1]   # Number of input nodes:  12*20+15*20=540 and 1 bias (bias comes at the 1st column)
    h1_size = 15                # Number of hidden nodes
    #h1_size = 12                # Number of hidden nodes
    h2_size = 300
    y_size = train_y.shape[1]   # Number of outcomes (recognize:1; non-recognize:0);
                                # can not show train_Y.shape[1] because equals=1 does not show, but if it is larger than 1 it could show
    ...
    updates = tf.train.GradientDescentOptimizer(0.001).minimize(cost) # original 0.01
